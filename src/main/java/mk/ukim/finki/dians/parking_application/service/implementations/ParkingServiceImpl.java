...
public class ParkingServiceImpl implements ParkingService {
    ...
    public List<Parking> findByCurrentAddress(Double currentLatitude, Double currentLongitude) {
        return parkingRepository.findAll().stream()
                .filter(parking ->
                        haversineDistance(parking.getLatitude(),
                                parking.getLongitude(), currentLatitude, currentLongitude)<3)
                .sorted(Comparator.comparing(parking ->
                        haversineDistance(parking.getLatitude(),
                                parking.getLongitude(), currentLatitude, currentLongitude)))
    ...
    @Override
    public Optional<Parking> save(Parking p) {
        return Optional.of(this.parkingRepository.save(p));
    }
